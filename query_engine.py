...
- CRITICAL: resolve relative season words against today's date above, and remember a
  season is named for the two calendar years it spans (Jul 15 of the first year to Jul 15
  of the second, matching the seasons seed table used elsewhere in the platform). "Last season" / "letzte Saison" means
  the most recently COMPLETED season, which is NOT simply "today's year minus one": on
  2026-08-27 the 2025-2026 season ended in June 2026 and the 2026-2027 season has barely
  started, so "last season" is 2025-2026 (date >= '2025-07-15' AND date < '2026-07-15'),
  not 2024-2025. Likewise
...
- bronze.* boxscore/playbyplay tables have NO season column, only a per-game date. If a question names a season or year and the query has no date/season filter, the query is wrong, not just imprecise - it will silently mix multiple seasons together.
- A competition season "YYYY-(YYYY+1)" runs from July 15 of YYYY to July 15 of YYYY+1, crossing the calendar-year boundary. "Season 2025-2026" and "season 2025" both mean this range - NEVER filter by calendar year (date BETWEEN '2025-01-01' AND '2025-12-31'), that silently cuts the season in half.
- For a bronze-only query scoped to season YYYY-(YYYY+1), filter with: date >= 'YYYY-07-15' AND date < '{{YYYY+1}}-07-15'. This matches the seasons seed table (e.g. 2025-2026 = 2025-07-15 to 2026-07-15) used by the gold-layer season column - a bronze query using a different boundary can silently disagree with the equivalent gold-table total even when both look like they're scoping the same season.
- gold.g_el_players, g_ec_players, g_cl_players, and g_bbl_players already have a season column formatted like '2025-2026' - prefer filtering there with season = '2025-2026' over date math whenever the requested stat exists in a gold table.
- CRITICAL: gold.g_el_players, g_ec_players, g_cl_players, and g_bbl_players (all four) name their player column "player", NOT "player_name" - verified against live data. This differs from every bronze boxscore/player_info table AND from gold.g_player_profile_* (all of which do use "player_name"), so the natural-language alias "player = player_name" below does NOT apply to these four specific tables. Using player_name against g_bbl_players (or any of its three siblings) raises a column-does-not-exist error - check the live column listing for the specific table in play rather than assuming the alias holds everywhere.
- CRITICAL: gold.g_player_profile_* tables (pts, ast, reb, etc.) store PER-GAME AVERAGES, not season totals, and carry NO minimum-games qualification - a player with gp=1 who scored 21 in that one game outranks players who averaged 19 across 37 games. A "top scorer"/"best"/"leading" question ranked by an unqualified per-game average is misleading, not just imprecise (verified against live data: EuroLeague 2025-26's #1 by raw pts is a 1-game player, not the real 39-game, 740-total-point scoring leader). Prefer adding a minimum games-played threshold ON THIS SAME TABLE (e.g. gp >= 20, roughly half a season) over switching to a different table - g_player_profile_* already has position/team/nationality alongside the stat in one row, so filtering it directly avoids needing any join at all. Only fall back to SUM(pts) from a bronze boxscore table if the question specifically needs a true season total rather than a qualified average - and if you do, filter player_name per the player-name-format note below before joining boxscore to any other table for extra attributes. TWO EXCEPTIONS to "prefer this table". (a) Because the columns are ALREADY per-game, NEVER divide them by gp - that yields a per-game-per-game figure that systematically favours players with few games; verified against live data, "trb / gp" picked the wrong best rebounder for 8 of 18 BBL teams, promoting a 16-game player who rebounds worse per game over a 21-game player who rebounds better. Rank on the column as it stands. (b) These tables can be INCOMPLETE relative to the bronze boxscore, so any question about coverage - "per team", "every player", "how many players" - is safer answered from bronze directly. The size of the gap varies by league and drifts as more games get crawled, so do not assume a fixed number - verified against live data for season 2025-2026: g_player_profile_championsleague (462) is missing ~15 of the 477 players bronze has, g_player_profile_eurocup (311) is missing ~7 of bronze's 318, while g_player_profile_bbl (325) and g_player_profile_euroleague (351) currently match or exceed bronze's own distinct-player count (311 and 351) - so a stale "this table is missing players" assumption for BBL/EL specifically can be just as wrong as trusting it blindly. For a genuine coverage question, verify against the current bronze count rather than relying on any previously-seen gap.
- CRITICAL: gold.g_player_profile_* tables (all 4 leagues) now also contain one extra row per player labeled season = 'Ø 3 Seasons' - a pre-aggregated 3-season summary row, not a real season, added alongside the normal per-season rows (verified live: gold.g_player_profile_bbl currently has 2024-2025, 2025-2026, and Ø 3 Seasons as its only three season values). This row's gp and counting-stat-derived columns are computed over 3 seasons combined, not one, so it does NOT belong in a single-season ranking or a raw MAX(pts)/ORDER BY pts query without a season filter - mixing it in silently compares a 3-season figure against single-season ones. Always filter to an explicit real season (season = '2025-2026') for a single-season question, and only include season = 'Ø 3 Seasons' when the question explicitly asks for a multi-season average or total.
...
- CRITICAL: *_player_info tables (position, nationality, height, birthday) have NO season or date column - they are a rolling multi-season/career player registry, not one season's roster, and the same player can appear multiple times across different crawled seasons or team changes. A question that names a season or says "this season" but is answered purely from *_player_info silently mixes every season the crawler has ever seen together instead of scoping to one - verified against live data: counting Champions League guards purely from b_cl_player_info gives 646 (all crawled seasons combined, drifts upward as more seasons get crawled), but the real 2025-26-season count, found by requiring the player to also appear in that season's date-filtered boxscore, is 212 - a ~3x inflation. Whenever a question about position/nationality/height/etc. also names a season, first find that season's actual player set from a season-dated source (bronze boxscore filtered by date, or a gold table with season) and only THEN join to player_info for the attribute - never query player_info alone when the question specifies a season. (player_name format usually matches between a league's own boxscore and player_info tables, but confirm before joining per the player-name-format note below - it is not guaranteed.)
- CRITICAL: player_name is stored as "First Last" (space-separated, no comma) in every *_boxscore and *_player_info table, across all 4 leagues - verified against live data: 0 of 75,210 combined boxscore rows and 0 of 3,789 combined player_info rows (BBL/EL/EC/CL) contain a comma in player_name. Never build a name filter assuming "Last, First" order (e.g. player_name ILIKE 'Obst A%' for "Andreas Obst") - it silently matches zero rows and returns NULL, which looks like missing data rather than a wrong filter, not an error you'd notice. Match a named player with both name parts in "First Last" order, e.g. player_name ILIKE '%Andreas%Obst%' or player_name ILIKE 'Andreas Obst%', never anchored to a comma or reversed order.
...
- CRITICAL: for BBL specifically, b_bbl_boxscore.player_name and b_bbl_player_info.player_name both use "First Last" format and match directly - verified against live data (both store "Ryan Mikesell" for the same player; 0 of 23,465 boxscore rows contain a parenthetical suffix). An earlier version of this note claimed boxscore.player_name looked like "Mikesell R. (SF)" (abbreviated, with a "(POS)" suffix) and had to be bridged through b_bbl_player_info.player_name_short to join at all - that claim did not match any real row and has been removed; player_name_short ("MIKESELL R.", all caps, no suffix) is a separate, player_info-only column, not the shape of boxscore's own player_name, and a direct join on player_name (case-insensitively) works. One real wrinkle: a small minority of boxscore rows (126 of 23,465, ~0.5%) instead use an abbreviated "Last F." form (e.g. "Anderson K."). If a "%First%Last%" ILIKE match returns zero rows for a named player, retry with this abbreviated form before concluding the player has no data - do not assume the query itself is wrong. Also note b_bbl_player_info can carry multiple, sometimes conflicting, position strings for the same player across different crawled records (e.g. one row says 'Small Forward (SF)', another says 'SF', another says 'Shooting Guard (SG)' for the same person) - use ILIKE matching (per the position-value note above) rather than expecting one canonical value per player.
...
- The tm-prefixed columns in silver.*_player_stats_prospiel (tm_pts, tm_ast, tm_orb, tm_drb, tm_stl, tm_tov) are the TEAM's totals accumulated only during this player's on-court minutes for that game - not this player's own individual stat (that is the bare pts/ast/etc. in bronze boxscore tables) and not the team's full-game total. Verified against live data: for a game where the team's real final total was 108 points and 15 assists, one rotation player's tm_pts was 95 and tm_ast was 14 - both slightly under the full-game figures, consistent with "team production while this player was on the floor" for most but not all of the game. Do not sum tm_pts across a player's games to get "team points" (it double counts every possession the team's other rotation players were also on court for) and do not treat it as the player's own scoring. The same-shaped opp-prefixed columns in this table (opp_pts, opp_ast, opp_drb, opp_orb, opp_tov) are the OPPONENT's totals at the identical on-court-subset grain, not the opponent's full-game total either - verified against live data for the same game: the real opponent final score was 85, this player's row shows opp_pts=73.
- CRITICAL: gold.g_bbl_players has TWO differently-scaled columns per rebounding side that are easy to grab by mistake for each other because their names look similar - orb_pct/drb_pct (this table, PLAYER-level individual rebound rate, on a 0-100 PERCENT scale) versus orb_pct/drb_pct (gold.g_*_teams, TEAM-level rebound rate, on a 0-1 FRACTION scale) are DIFFERENT metrics on DIFFERENT tables with DIFFERENT scales, not the same stat at two grains. Also note this table's player-name column is called "player", not "player_name" - a query against g_bbl_players using player_name raises a column-does-not-exist error. Verified against live data: David McCormack's (gold.g_bbl_players) drb_pct is 23.656 for season 2025-2026 (percent scale, matches his known ~23.8% defensive rebound rate); FC Bayern München's (gold.g_bbl_teams) orb_pct is a fraction like 0.334 for the same season. Mixing these up doesn't error, it silently produces a number ~100x off or a wrong-grain answer (a player's own rate reported for a question about the team, or vice versa).
- bronze.b_bbl_wurfposition uses xcoordinate/ycoordinate (no underscore) while bronze.b_el_wurfposition uses x_coordinate/y_coordinate (with underscore) for the same concept - using the wrong league's spelling raises a column-does-not-exist error rather than returning a wrong answer, but check which wurfposition table is in play before writing the column name from memory. (There is no silver.*_wurfposition table for either league - both leagues' shot-position data only exists in bronze; an earlier version of this note claimed a silver.b_el_wurfposition table, which does not exist.)
...
- CRITICAL: this rename split is NOT limited to bronze - gold.g_el_teams, g_ec_teams, g_bbl_teams, etc. (pre-aggregated per-team-season rate stats like ppp) can ALSO have two separate rows for the same team+season, one per name variant, and these are RATE stats you cannot simply SUM to combine - you must weight by possessions (poss). Verified against live data: gold.g_el_teams has both 'Crvena Zvezda Meridian Belgrade' (70.0 possessions/game, an incomplete partial-season split) and 'Crvena Zvezda Meridianbet Belgrade' (74.974 possessions/game) as separate rows for season 2025-2026 - picking either row alone for a "best net rating" question gives a wrong team AND a wrong number: the properly weighted combined net rating (0.080) is actually LOWER than Olympiacos Piraeus's 0.108, meaning Olympiacos is the real answer, not Crvena Zvezda's misleadingly high 0.159 from the small-sample split alone. Before ranking teams by any gold.g_*_teams column, check whether any team name appears more than once for the same season (a quick GROUP BY team, season HAVING COUNT(*) > 1 first); if so, merge the rate columns with a possession-weighted average (SUM(rate * poss) / SUM(poss)) before ranking, don't just pick one row.
- CRITICAL: BBL bronze tables (b_bbl_boxscore, b_bbl_playbyplay) have a competition column - 'easyCredit BBL' for real league games, 'Pokal' for cup games. This is NOT only a team-level concern - filter with competition = 'easyCredit BBL' for ANY BBL aggregation, team-level ("which team scored the most", "how many teams", "bottom N teams", averages across teams) OR player-level ("best shooting percentage", "top scorer", "most rebounds", any player leaderboard or per-player stat total/average). Cup games inflate and distort both: team-level, ProA cup opponents that only appear 1-2 times all season (vs. 33+ for a real BBL team) get silently counted as BBL teams (24 raw team names vs. the real ~17-18); player-level, cup-game stats silently get mixed into a player's season totals and can change who ranks first. Verified against live data, BBL season 2025-2026, "best field goal percentage, minimum 50 attempts": unfiltered top is Onu E., 137/201 (68.16%); filtered to competition = 'easyCredit BBL' only, the correct answer is Onu E., 119/173 (68.79%) - a different make/attempt count and percentage for the SAME player, from the same query minus this one filter. Also verified team-level: filtering by competition = 'easyCredit BBL' produces the exact same 18-team set as the older row-count heuristic (HAVING COUNT(*) > 2 on the team-game grouping) - use the column, it's the direct signal rather than an inferred proxy. Do not skip this filter just because a question is about a player rather than a team.
...
- If the question names a season, year, or date range, the query MUST include a matching WHERE filter on date (or season, for gold tables) - see "Season and date filtering" above. Do not drop a time constraint just because it makes the query simpler.
