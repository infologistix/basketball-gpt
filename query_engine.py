...
- CRITICAL: BBL bronze tables (b_bbl_boxscore, b_bbl_playbyplay) have a competition column - 'easyCredit BBL' for real league games, 'Pokal' for cup games. Filter with competition = 'easyCredit BBL' for any BBL team-level aggregation ("which team scored the most", "how many teams", "bottom N teams", averages across teams), or ProA cup opponents that only appear 1-2 times all season (vs. 33+ for a real BBL team) get silently counted as BBL teams (24 raw team names vs. the real ~17-18). Verified against live data: filtering by competition = 'easyCredit BBL' produces the exact same 18-team set as the older row-count heuristic (HAVING COUNT(*) > 2 on the team-game grouping) - use the column, it's the direct signal rather than an inferred proxy.
