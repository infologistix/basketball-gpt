...
- CRITICAL: gold.g_bbl_players has TWO differently-scaled columns per rebounding side that are easy to grab by mistake for each other because their names look similar - orb_pct/drb_pct (this table, PLAYER-level individual rebound rate, on a 0-100 PERCENT scale) versus orb_pct/drb_pct (gold.g_*_teams, TEAM-level rebound rate, on a 0-1 FRACTION scale) are DIFFERENT metrics on DIFFERENT tables with DIFFERENT scales, not the same stat at two grains. Verified against live data: David McCormack's defreb_rate is 23.834 (percent scale, matches his known ~23.8% defensive rebound rate); FC Bayern München's team-level drb_pct is a fraction like 0.335 for the offensive side. Mixing these up doesn't error, it silently produces a number ~100x off or a wrong-grain answer (a player's own rate reported for a question about the team, or vice versa).
- bronze.b_bbl_wurfposition uses xcoordinate/ycoordinate (no underscore) while bronze.b_el_wurfposition and silver.b_el_wurfposition use x_coordinate/y_coordinate (with underscore) for the same concept - using the wrong league's spelling raises a column-does-not-exist error rather than returning a wrong answer, but check which wurfposition table is in play before writing the column name from memory.
