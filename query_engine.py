...
- CRITICAL: resolve relative season words against today's date above, and remember a
  ...
  of the second, matching the seasons seed table used elsewhere in the platform - NOT
  Aug 1, a mismatch that previously existed here). "Last season" / "letzte Saison" means
...
- For a bronze-only query scoped to season YYYY-(YYYY+1), filter with: date >= 'YYYY-07-15' AND date < '{{YYYY+1}}-07-15' (use date_final for BBL). This matches the seasons seed table (e.g. 2025-2026 = 2025-07-15 to 2026-07-15) used by the gold-layer saison column - a bronze query using a different boundary can silently disagree with the equivalent gold-table total even when both look like they're scoping the same season.
...
- CRITICAL: gold.g_bbl_players has TWO differently-scaled columns per rebounding side that are easy to grab by mistake for each other because their names look similar - ofreb_rate/defreb_rate (this table, PLAYER-level individual rebound rate, on a 0-100 PERCENT scale) versus of_per_eigenes/def_per_eigenes (gold.g_*_teams, TEAM-level rebound rate, on a 0-1 FRACTION scale) are DIFFERENT metrics on DIFFERENT tables with DIFFERENT scales, not the same stat at two grains. Verified against live data: David McCormack's defreb_rate is 23.834 (percent scale, matches his known ~23.8% defensive rebound rate); FC Bayern München's team-level def_per_eigenes is a fraction like 0.335 for the offensive side. Mixing these up doesn't error, it silently produces a number ~100x off or a wrong-grain answer (a player's own rate reported for a question about the team, or vice versa).
- CRITICAL: within gold.g_bbl_players, columns ending in _rate do NOT share one numeric scale - verified against live data for one player: tmast_rate=0.498 and tmtov_rate_eigenes=0.151 are 0-1 fractions, tm_twop_rate=57.004 and tm_threep_rate=37.861 are 0-100 percentages, while tmstl_rate=11.523 and tm_ft_trip_rate=14.624 are a per-100-possessions-style rate on neither of those scales. Never assume a common scale across different _rate columns, even within the same table or the same tm-prefixed family - check a sample value's plausible range (0-1, 0-100, or a rate-per-100) before using one in a calculation, comparison, or threshold.
